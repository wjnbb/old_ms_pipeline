...
calcs("PA14", "20221013_PA14", 2, 3)

strain <- "Josh_EtOAc_WEs"
batch <- "20230124_Josh_EtOAc_WEs"
fcg_num <- 1
FC_threshold <- 3

calcs <- function(strain, batch, fcg_num, FC_threshold){
...
for(g in groups){
  ...
  g_cols <- grep(paste0(g, "_"), colnames(mzmine))
...
#calculate the averages for the whole row regardless of groups and use this for FC calcs etc
total_mean <- rowMeans(intensities)
total_sd <- apply(intensities, 1, sd)
total_rsd <- (total_sd/total_mean)*100

mzmine2 <- add_column(mzmine2, total_mean, .before = colnames(mzmine2)[7])
mzmine2 <- add_column(mzmine2, total_sd, .before = colnames(mzmine2)[8])
mzmine2 <- add_column(mzmine2, total_rsd, .before = colnames(mzmine2)[9])
...
for (g in groups) {
  ...
  #Filter the data by FC and RSD using subset
  mzmine_filt <- subset(mzmine_filt, mzmine_filt[,5] >= FC_threshold &
                          mzmine_filt[,4] <= 30)
