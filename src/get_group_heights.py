...
def get_single_group_areas(
    area_df: pd.DataFrame, names: list
):
    ...
    if MS_system == "Sciex":
        ...
        # Add datafile: to the start of the sample name
        Media_cols = ["datafile:" + c for c in Media_cols]
        # Add :area to the end of the sample name
        Media_cols = ["{}{}".format(c, ":area") for c in Media_cols]
        # Get the sample group only peak height table
        Media_table = area_df.loc[:, Media_cols]
    ...
    if MS_system == "Sciex":
        ...
        Bio_cols = ["datafile:" + c for c in Bio_cols]
        # Add :height to the end of the sample name
        Bio_cols = ["{}{}".format(c, ":area") for c in Bio_cols]
        # Get the sample group only peak height table
        Bio_table = area_df.loc[:, Bio_cols]
    ...
    if MS_system == "Sciex":
        ...
        Blank_cols = ["datafile:" + c for c in Blank_cols]
        # Add :area to the end of the sample name
        Blank_cols = ["{}{}".format(c, ":area") for c in Blank_cols]
        # Get the sample group only peak height table
        Blank_table = area_df.loc[:, Blank_cols]
    ...
    if MS_system == "Sciex":
        ...
        QC_cols = ["datafile:" + c for c in QC_cols]
        # Add :area to the end of the sample name
        QC_cols = ["{}{}".format(c, ":area") for c in QC_cols]
        # Get the sample group only peak height table
        QC_table = area_df.loc[:, QC_cols]
...
def get_group_heights_binary(
    ...
    #Get sample names for the group from the identifier
    Blank_names = [c for c in names if Bio_identifier not in c]
