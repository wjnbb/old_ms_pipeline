...
def divide_mzmine3_table(peak_table, pt_cols):
    ...
    intensity_range_cols = peak_table[(c for c in pt_cols if c.startswith('intensity_range:'))]
    aligned_feature_info = pd.concat([aligned_feature_info,
                                      rt_range_cols,
                                      mz_range_cols,
                                      intensity_range_cols], axis=1)
    ...
    MS1_match_data = peak_table[(c for c in pt_cols if c.startswith('compound_db_identity:'))]
    MS2_match_data = peak_table[(c for c in pt_cols if c.startswith('spectral_db_matches:'))]
    MF_pred_data = peak_table[(c for c in pt_cols if c.startswith('formulas:'))]
    Alignment_data = peak_table[(c for c in pt_cols if c.startswith('alignment_scores:'))]
    IIN_data = peak_table[(c for c in pt_cols if c.startswith('ion_identities:'))]
    manual_anno_data = peak_table[(c for c in pt_cols if c.startswith('manual_annotation:'))]
    ...
    return MS1_match_data, MS2_match_data, MF_pred_data, Alignment_data, IIN_data, manual_anno_data, height_df, aligned_feature_info
